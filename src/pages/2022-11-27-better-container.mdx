...
title: ブロックエディタ向けのCSSを参考にした、よりよいコンテナの実装
...
ここまでは以前の記事で紹介したやり方と変わらない。しかし、コンテナの外側にできる余白の実装が少し違う。
...
最大幅の実装は個々の要素が担いつつ、外側の余白は親要素に持たせておく。ページいっぱいまでに広がる要素（alignfull）は例外として、ネガティブマージンで処理する。
...
これらの例はWordPress向けの実装になっているので、少し手を加えて、実装パターンとして単独で成立するように整理した。
...
.entry-content > :not(.alignwide, .alignfull) {
	max-block-size: var(--content-size);
}

.entry-content > .alignwide {
	max-block-size: var(--wide-size);
}

.entry-content > .alignfull {
	margin-inline: calc(-1 * var(--spacing-outer));
}
```
