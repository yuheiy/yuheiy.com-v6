...
ただし、このような実装ではフォーカスさせることができずに失敗してしまう。なぜなら、スタイルを書き換えた直後にはまだ不可視状態（`visibility: hidden`）のままだからだ。フォーカスさせるには、アニメーションが開始して`visibility: visible`の状態になるまで待機しなければならない。その対処として多くの場合、`requestAnimationFrame`メソッドが使用されがちである。
